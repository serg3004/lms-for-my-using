...
4. Перед PR 267–281 проверить TV-020/TV-056. Если owner decision всё ещё не закрыт — остановить реализацию PR 267–281 и пометить блокировку фактом.
5. PR 282–284 не считать автоматически одобренными решением по текущему TV-056, если в `TODO_VERIFY.md` оно по-прежнему охватывает только PR 266–281. Перед PR 282–284 требуется отдельное явное owner approval либо расширение зафиксированного owner decision до PR 266–284.
6. Не начинать PR, если его функциональная основа из предыдущих PR ещё не находится в `main`.
7. Не менять целевую архитектуру самовольно. Если текущий код делает запланированное решение невозможным или business outcome неоднозначен — зафиксировать конфликт и запросить owner decision.
8. Каждый PR должен быть атомарным: только код, schema, tests и docs, относящиеся к его задаче.
9. Изменение behavior требует tests. Изменение API/schema/RBAC/migration требует соответствующего обновления документации.
10. Не ослаблять security/tests ради зелёного CI.
11. Не объявлять PR завершённым без проверки всех критериев оценки и доступного CI.
12. После каждого schema PR проверять clean migration и representative upgrade path, если это применимо.
13. После появления реальных данных rollback предпочитает application rollback + сохранение additive schema/data + forward-fix, а не destructive DROP.
14. Если план и текущий код расходятся:
...
- При первом появлении archive API зафиксировать structural invariant:
  - Department с active child нельзя архивировать;
  - операция должна вернуть conflict (`409`) и потребовать сначала move/archive children;
  - active tree/path/search не должны оставлять активный descendant под archived ancestor.
...
- [ ] Department с active child нельзя архивировать;
- [ ] после archive в active tree/path/search не остаётся active descendant под archived ancestor;
...
- Реализовать tree table с данными, которые уже предоставляет PR 269:
  - Department;
  - code;
  - type;
  - actions.
- Не требовать и не подменять fake data для ещё не существующих backend contracts:
  - `manager summary` добавить в PR 273 после DepartmentManager API;
  - `users count` добавить в PR 274 после headcount API.
- Layout может резервировать место под будущие колонки только как явно скрытый/disabled placeholder без выдуманных значений.
- Реализовать:
...
- [ ] PR 270 не требует manager summary/users count от API PR 269 и не показывает fake values;
...
- Добавить tenant integrity только для существующих на этом этапе User/Department relations.
- `positionId` и Position tenant integrity намеренно не добавлять в PR 271: они вводятся атомарно вместе с `Position` в PR 275.
- Добавить API:
...
- [ ] PR 271 не создаёт преждевременную Position relation или constraint;
...
- В tree table добавить отложенную из PR 270 колонку/summary руководителей и показывать:
...
- Создать Department users page:
  - server search;
  - pagination;
  - до PR 275 не показывать реальные Position data; допускается только явно скрытый/disabled placeholder без fake values;
...
- [ ] до PR 275 UI не требует Position API/data и не показывает fake Position values;
...
- В tree UI добавить отложенную из PR 270 колонку `users count`, используя реальный headcount API этого PR без client-side пересчёта.
...
- [ ] tree UI показывает users count из backend headcount API, без fake/client-side aggregate;
...
- Добавить `positionId?` в `DepartmentMembership` именно в этом PR.
- Добавить tenant integrity DepartmentMembership ↔ Position и same-tenant validation/DB defense, где это поддерживает текущая schema strategy.
...
- [ ] Position relation и tenant integrity добавлены только после создания Position model;
...
- Валидации:
  ...
  - dates;
  - file/row bounds.
- Расширить archive lifecycle, сохраняя введённое в PR 269 правило `active children → conflict`:
...
- Перед началом проверить отдельное явное owner approval для PR 282–284 либо зафиксированное расширение TV-056/эквивалентного owner decision до диапазона PR 266–284.
...
- [ ] отдельное owner approval для PR 282–284 подтверждено до начала реализации;
