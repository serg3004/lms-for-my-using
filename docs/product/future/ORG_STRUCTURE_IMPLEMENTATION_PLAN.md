...
**Статус: реализовано.** Добавлена модель `ReportingLine` (`id`, `organizationId`, `employeeId`, `managerId`, `type` (`DIRECT`/`FUNCTIONAL`/`PROJECT`), `isPrimary`, `effectiveFrom`/`effectiveTo?`) и модуль `reporting-lines` с указанным API: `GET users/:id/reporting-lines`, `GET users/:id/effective-manager`, `POST reporting-lines`, `PATCH reporting-lines/:id`, `POST reporting-lines/:id/close` — RBAC admin-only, тот же паттерн, что и `department-managers`. `employeeId == managerId` отклоняется и Zod-refine, и DB CHECK constraint (`reporting_lines_employee_not_manager_check`). Два partial unique index (raw SQL в миграции, по аналогии с `department_managers`): `reporting_lines_current_employee_manager_type_key` (запрещает дублирующую current (employee, manager, type) строку) и `reporting_lines_current_primary_type_key` (не более одного current primary manager на (employee, type)) — это позволяет matrix-организацию (несколько current DIRECT-менеджеров одного сотрудника у разных людей), но только один может быть primary. Cycle detection реализован только для `DIRECT`-рёбер (единственный тип, формирующий граф иерархии, который обходит `OrganizationAccessScopeService` и `getEffectiveManager`): `wouldCreateDirectReportingCycle` (recursive CTE, `reporting-line-queries.ts`) проверяет, не является ли `managerId` уже (прямым или транзитивным) репортом `employeeId` — если да, новое ребро замкнуло бы цикл; проверка выполняется внутри Serializable-транзакции с bounded retry (`runSerializableWithRetry`, переиспользован из `departments`), тот же паттерн, что и cycle-check при reparent Department из PR 269; проверено выделенным database-integration тестом с двумя параллельными create, которые вместе образовали бы цикл — ровно один коммитится. `FUNCTIONAL`/`PROJECT` рёбра создаются без cycle-check (не участвуют в этом графе). `GET users/:id/effective-manager` реализует ровно указанный в плане fallback: current primary `DIRECT` ReportingLine → иначе current primary effective `DIRECT` DepartmentManager текущего primary Department (переиспользован `computeEffectiveDepartmentManagers` из PR 272 через новый `department-managers/public.ts`) → иначе `null`. `OrganizationAccessScopeService` (PR 278) получил третью union-ветку: `directReportIds` — прямые и транзитивные `DIRECT`-репорты актора (`getTransitiveDirectReportIds`, recursive CTE), гейтится тем же требованием `RBAC role = manager`, что и Group/Department ветки, дедуп через `OR`, никогда не расширяется `FUNCTIONAL`/`PROJECT`-связями или строками из чужого tenant (каждый запрос строго `organizationId`-scoped). `PATCH reporting-lines/:id` сознательно ограничен обновлением только `isPrimary` — смена `employeeId`/`managerId`/`type` моделируется как close + create новой линии, чтобы не пришлось повторно гонять cycle detection на update. Каждая мутация пишет `OrgStructureEvent` (`reporting_line.created/updated/closed`) в той же транзакции. Authorization regression покрыт `apps/api/src/integration/reporting-lines.database.spec.ts` (real Postgres): self-manager, duplicate current/primary, FUNCTIONAL/PROJECT хранятся отдельно без cycle-check, DIRECT cycle отклонён, конкурентный cycle-race (ровно один коммитится), effective-manager fallback (все три ветки), scope по direct+транзитивным DIRECT-репортам с RBAC-гейтом и без расширения FUNCTIONAL/PROJECT, foreign-tenant employee отклонён 404, OrgStructureEvent пишется на create/update/close. Frontend UI в профиль пользователя не добавлен в этом PR — область PR 279 сознательно ограничена backend RBAC/scope-архитектурой (по аналогии с PR 278, где UI-фильтр отчётов тоже был отложен); API полностью функционален и покрыт тестами, UI — последующая задача. Backend unit (`reporting-lines.service.spec.ts`, обновлённые `organization-access-scope.service.spec.ts`/`manager.service.spec.ts`/`reports.service.spec.ts`) и database-integration suites зелёные локально против реального Postgres; `tsc --noEmit`, `eslint`, `architecture:check` и `CI=true node scripts/check-critical-coverage.mjs` проходят без регрессий.
