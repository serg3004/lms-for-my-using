...
## 25. Настраиваемые чек-листы и проверки на рабочем месте

> Сегодня `Lesson.type` допускает значения `checklist`/`workplace_check` только как строковую метку в UI (`apps/web/src/app/AdminLessonsPage.tsx`) и в Zod-схеме (`apps/api/src/modules/lessons/lessons.schemas.ts`) — реального поведения за этим нет: такой урок проходит как обычный `Lesson` с общим `Progress`, без пунктов, критериев, баллов или отдельного назначения. Ниже — спецификация двух самостоятельных сущностей по паттерну Template/Instance (аналогично `Assessment`/`AssessmentAttempt`), прототипы экранов конструктора: `docs/lms-ui-prototypes-complete/admin/lms-admin-checklist-builder-verified.html` и `docs/lms-ui-prototypes-complete/admin/lms-admin-workplace-check-builder-verified.html`.

### 25.1 Checklist (шаблон чек-листа)
- **Модель данных:** `Checklist — id, organizationId, title, description, competencyId (FK, nullable), scoringMode (sumPoints/allRequired), passThreshold (% или null для allRequired), assignmentMode (auto/scheduled/onDemand), assignmentTrigger (json — событие: назначение должности, завершение задачи урока и т.д., актуально при auto), assignmentSchedule (cron-подобное правило, актуально при scheduled), dueDuration (интервал на выполнение от момента назначения), status (draft/published/archived), createdBy, createdAt, updatedAt`. `ChecklistItem — id, checklistId (FK), order, text, points (int, актуально при scoringMode=sumPoints), isRequired (bool, актуально при scoringMode=allRequired), photoRequired (bool), attachmentRef (nullable — образец/иллюстрация к пункту, задаётся автором шаблона)`.
- **Instance:** `ChecklistInstance — id, checklistId (FK), userId (кому назначено), assignedAt, assignedReason (auto-trigger id / schedule run id / manual), dueAt, status (assigned/in_progress/completed/expired), completedAt, totalScore, passed (bool)`. `ChecklistItemResult — id, checklistInstanceId (FK), checklistItemId (FK), checked (bool), checkedBy (userId — сам learner или наставник за него), checkedAt, photoRef (nullable — фактическое фото-подтверждение по пункту), comment (nullable)`.
- **RBAC:** admin/instructor (автор) — CRUD шаблона, публикация; manager — назначение вручную (`onDemand`)/просмотр результатов подчинённых; learner — отмечает пункты своего `ChecklistInstance`, прикрепляет фото; наставник (роль instructor, назначенный на конкретного learner) — может отмечать пункты за learner, если это разрешено политикой организации; чтение чужих `ChecklistInstance` запрещено всем, кроме admin/manager в рамках своей организации.
- **Edge cases:** шаблон отредактирован (изменены пункты/баллы) после того, как по нему уже есть активные `ChecklistInstance` — редактирование должно создавать новую версию шаблона или блокироваться, иначе баллы уже начатых инстансов станут несопоставимы; `photoRequired=true`, но у learner нет возможности загрузить фото (нет камеры/офлайн) — нужен резервный путь (пометка «недоступно» с обязательным комментарием, видимым проверяющему), а не блокировка прохождения навсегда; `scoringMode=sumPoints` и `passThreshold` не достигнут при всех отмеченных пунктах — `ChecklistInstance.passed=false`, инстанс не может быть переотмечен без создания нового инстанса (переназначения), чтобы сохранить историю попытки; `assignmentMode=auto` триггерится повторно для того же пользователя, пока предыдущий `ChecklistInstance` ещё не завершён — дедупликация, не создавать второй активный инстанс; `dueAt` истёк без завершения — статус `expired`, отдельно от `completed`, чтобы отчётность различала «не сделал» и «сделал с опозданием»; удаление пункта из шаблона, у которого уже есть `ChecklistItemResult` в истории — soft-delete пункта (`archivedAt`), а не физическое удаление, чтобы не терять историю уже пройденных инстансов.

### 25.2 Workplace Check (шаблон проверки на рабочем месте)
- **Модель данных:** `WorkplaceCheck — id, organizationId, title, description, competencyId (FK, nullable), evaluatorRole/evaluatorUserId (кто уполномочен оценивать — роль или конкретный instructor/manager), scoringMode (rubric/passFail), passThreshold (% при rubric), minCriterionFloor (nullable int — минимальный балл по каждому критерию, ниже которого автоматический fail независимо от суммы), assignmentMode (auto/scheduled/onDemand), assignmentTrigger/assignmentSchedule/dueDuration (аналогично 25.1), eligibilityRule (json — кто может подать заявку, актуально при onDemand), retryPolicy (immediate/afterCooldown(duration)/manualApprovalRequired — что происходит при провале), status (draft/published/archived), createdBy, createdAt, updatedAt`. `WorkplaceCheckCriterion — id, workplaceCheckId (FK), order, text, scaleMax (например 5), weight (множитель для rubric), photoSample (nullable — образец «как должно быть», задаётся автором шаблона)`.
- **Instance:** `WorkplaceCheckInstance — id, workplaceCheckId (FK), userId (проверяемый), evaluatorUserId (кто фактически оценил), assignedAt, scheduledAt (nullable — для scheduled/onDemand это дата фактической проверки на месте, отличная от assignedAt), dueAt, status (assigned/scheduled/in_progress/completed/failed/expired), completedAt, totalScore, passed (bool)`. `WorkplaceCheckCriterionResult — id, workplaceCheckInstanceId (FK), criterionId (FK), score (int, 0..scaleMax), photoRef (nullable — фактическое фото-подтверждение оценщика), comment (nullable, обязателен при низком score)`.
- **RBAC:** admin/instructor (автор) — CRUD шаблона, публикация, назначение `evaluatorRole`; evaluator (instructor/manager, согласно `evaluatorRole`/`evaluatorUserId` конкретного инстанса) — единственный, кто может проставлять `WorkplaceCheckCriterionResult` (learner не может сам себя оценить — в отличие от 25.1); learner — видит только свой `WorkplaceCheckInstance` и итоговый результат, может подать заявку при `assignmentMode=onDemand`, если проходит `eligibilityRule`; manager — просмотр результатов подчинённых, назначение вручную.
- **Edge cases:** `evaluatorUserId` уволен/сменил роль после того, как ему назначены незавершённые `WorkplaceCheckInstance` — нужна переназначение evaluator'а, иначе проверка зависает без ответственного; один критерий оценён ниже `minCriterionFloor` при в целом достаточной сумме баллов — итог всё равно `passed=false`, UI должен явно показать, какой именно критерий провалил проверку, а не просто общий fail; `assignmentMode=onDemand`, но learner не проходит `eligibilityRule` (например, недостаточно опыта/не пройден предшествующий Checklist) — заявка блокируется на уровне API с понятной причиной отказа, а не просто скрывается кнопка в UI; `retryPolicy=afterCooldown` — повторная заявка/переназначение до истечения cooldown отклоняется автоматически; при `failed` создаётся новый `WorkplaceCheckInstance` для повторной попытки, старый сохраняется в истории (append-only) — нельзя перезаписывать результат проваленной проверки; фото прикреплено оценщиком к критерию, но позже оспаривается learner'ом — комментарий/спор фиксируется как отдельная запись, а не редактированием существующего `score`/`photoRef` задним числом, чтобы сохранить аудируемость проверки.
