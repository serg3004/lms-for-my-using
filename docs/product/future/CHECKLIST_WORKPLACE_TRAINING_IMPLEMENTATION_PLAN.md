...
**Основание:** прототип `CHECKLIST_WORKPLACE_TRAINING_PROTOTYPE_V3.html` (архивирован в `docs/archive/old-trackers/` после полной реализации плана — см. `docs/archive/README.md`) и проверенные контракты репозитория.
**Статус:** реализация начата. PR 285 (архитектурные/продуктовые контракты) и PR 286 (organization-level настройки) выполнены. PR 287 (object-level authorization) частично: существующие checklist review/analytics эндпоинты переведены на `OrganizationAccessScopeService`, а `ChecklistSession` policy-функция (`sessionScope()`) определена и unit-протестирована. PR 288 (Prisma domain model для `ChecklistSession`) реализован. PR 289 (session lifecycle) реализован: `ChecklistSessionService` (`apps/api/src/modules/checklists/checklist-session.service.ts`) — серверная state machine (`scheduled -> in_progress -> paused -> in_progress -> completed`, плюс `scheduled -> cancelled`), optimistic-concurrency переходы через `version` (409 на stale write, отдельно от 400 на invalid transition), Serializable-транзакции с retry (`runSerializableWithRetry`, переиспользован из `departments/public.ts`, не скопирован), append-only `ChecklistSessionEvent`, запрет смены участников после старта. `sessionScope()` (PR 287) теперь реально вызывается из production-кода — маршруты `/checklist-sessions*` (`checklistSessionsRead`/`checklistSessionsManage`/`checklistSessionsRun` в `roles.ts`) — это закрывает предыдущую оговорку про "enforcement подтверждается только по мере появления реальных эндпоинтов". PR 290 (Criteria/Skip/scoring v1/фото/геолокация) реализован: skip-эндпоинт и weight/answerState-scoring встроены в существующий `ChecklistItemResult`/`ChecklistInstance` пайплайн (без новых таблиц результатов), `ChecklistInstance.scored` различает "all-skipped/not_scored" от настоящего 0%, фото-pipeline переиспользован без изменений, геолокация — `POST/GET /checklist-sessions/:id/location(/...)` с create-only "start/end only" (409 на дубликат), `off`-policy отказом и audited admin override. `ChecklistScale`-маппинг из этого PR намеренно исключён — `ChecklistSession` пока не ссылается на `ChecklistScale`, это остаётся за PR 293. PR 291 (Scheduler/reminders/notifications) реализован: `ChecklistSessionReminderWorker` — recurring job-сосед `ChecklistDeadlineWorker` на отдельном job name/scheduler id; `pre_start` (24h до `scheduledAt`) и `incomplete_after_start` (24h после `startedAt`) напоминания создаются/двигаются/подавляются хуками в `ChecklistSessionService.create/update/transition`; сам воркер — первый реальный consumer связки `Notification`+`OutboxEvent` в проекте, с business-идемпотентностью на уровне БД (условный `updateMany(...WHERE status='pending')`, а не только dedupe очереди); email — provider-neutral webhook (`ChecklistSessionReminderDelivery`, зеркалит `PasswordResetDelivery`), отсутствие конфигурации — валидный no-op. PR 292 (Admin API) реализован: bulk create (`POST /checklist-sessions/bulk`, per-recipient partial success через существующий `ChecklistsService.assignChecklist`), repeat (`POST /checklist-sessions/:id/repeat`, свежая пара instance+session, учитывает независимость session/instance lifecycle — completed session не значит completed instance), participant lookup (`GET /checklist-sessions/participants`, learner-lookup team-scoped через новый `ChecklistReviewAccessService.participantLearnerScope()`), published checklist lookup (`GET /checklists?status=published`), result projection (`list()`/`get()` теперь джойнят `checklist`/`learner`/`observer`/`result` из `ChecklistInstance`) и расширенные list-фильтры (`checklistId`/`learnerId`/`scheduledFrom`/`scheduledTo`/`search`) — без новых таблиц и новых role policies (переиспользованы `checklistSessionsManage`/`checklistSessionsRead`/`checklistsRead`). PR 293 (Evaluation Scales) реализован: `ChecklistItem.scaleId` (новая миграция `20260923120000_add_checklist_item_scale_ref`; сами таблицы `ChecklistScale`/`ChecklistScaleLevel` существовали с PR 288) — опциональная per-criterion ссылка на переиспользуемую шкалу, независимая от checklist-level `scoringMode`/`Checklist.scaleLevels`; `ChecklistScaleService` (create/update/archive, без RBAC-политик — переиспользованы `checklistsRead`/`checklistsCreate`); update отклоняется, если шкала уже используется хотя бы одним assigned-инстансом (archive — единственный разрешённый переход в этом случае, всегда неразрушителен); scale резолвится в `ChecklistScale`/`ChecklistScaleLevel` ровно один раз при `assignChecklist`/`bulkAssignChecklist` и встраивается в `templateSnapshot` — все читатели дальше используют только снэпшот, никогда live-таблицу; публикация чек-листа с критерием на архивную шкалу отклоняется, но уже опубликованные чек-листы не затрагиваются последующей архивацией. PR 294 (UI Foundation & Visual Contract) реализован: `Badge` расширен нейтральными тонами `success`/`info`/`danger`, `checklistStatus.ts` — единый источник статус→тон mapping для инстансов, `ADR_CHECKLIST_SESSION_OVERLAY.md` зафиксировал shell/tabs/dialog/breakpoints/responsive-table решения для всех следующих экранов. PR 295 (Admin: список сессий и wizard создания) реализован: `AdminChecklistSessionsPage` — под-маршрут `/admin/checklists/sessions` (не новый top-level nav-item), переиспользует `AdminPageLayout`/`AdminPageHeader`/`DataTable`/`Pagination`; новый `ChecklistsTabs` (переиспользует `OrgStructureTabs`-паттерн, не ARIA-tablist) связывает список чек-листов и список сессий; серверные фильтры (status/search/pagination) через уже существующий `GET /checklist-sessions`; wizard создания построен на новых `Dialog`/`WizardDialog` primitives в `shared/ui.tsx` (генерализация `ConfirmDialog`'s `ds-dialog` shell, как и требовал ADR PR 294, а не третья реализация модалки) — 4 шага (участники/лист/время и место/подтверждение) с невозможностью продвинуться на невалидном шаге (`WizardDialog`'s новый `nextDisabled`), bulk-создание через `POST /checklist-sessions/bulk` и опциональный auto-start цепочкой `GET .../:id` + `POST .../:id/start` для «Начать сейчас» (атомарного create-and-start эндпоинта в API нет). Намеренно не реализовано в PR 295 (честно задокументированный gap, не блокирует остальной workstream): индивидуальное расписание на сотрудника в bulk-режиме (backend поддерживает только одно общее время на batch), выбор часового пояса (используется только определённый браузером, без picker'а), поле «продолжительность» из прототипа (нет backend-поля), детальный экран отчёта по сессии (`▤ Отчёт`, отдельный PR), drawer «Настройки» (уже есть своя точка входа из PR 286), обнаружение конфликтов расписания наблюдателя (backend-концепции не существует). PR 296 (Observation Sheet Builder) реализован: расширяет существующий `ChecklistBuilder` (не новый URL-маршрут) новой моделью `ChecklistItemGroup` (add/rename/copy/reorder, `SetNull` FK, без delete-эндпоинта), `Checklist.contextFields`/`defaultLocationCapturePolicy`/`preSessionVisibility` (все резолвятся в `templateSnapshot` на момент назначения, `CHECKLIST_SNAPSHOT_VERSION` не увеличена), критерии получили per-item `weight`/`scale`-select (впервые дав frontend PR 293's `ChecklistScale`-библиотеке — новый `ScaleManagerDialog`, secondary/contextual `Dialog`). Никаких новых role policies. PR 297 (Observer: mobile-first проведение) реализован: расширяет существующий `/instructor/checklists` (`InstructorChecklistReviewsPage`) новой вкладкой «Проведение» (`ChecklistSessionsToConduct` — список собственных сессий наблюдателя через уже существующий `GET /checklist-sessions`, `sessionScope()` для чистого `instructor` уже равен `{observerId: user.id}`, поэтому дополнительный фильтр не нужен), открывающей mobile-first `ChecklistSessionConduct` — новая роль/страница/URL-маршрут не созданы (тот же паттерн in-page state, что у существующего `ReviewDetail`). Экран проведения: mobile-заголовок (аватар/имя/чек-лист/статус/таймер), progress bar по обязательным критериям, пошаговый Back/Next по критериям с карточкой критерия (snapshot-шкала `item.scale` при её наличии, иначе checklist-level `scoringMode`), комментарий с явным сохранением, Skip (только если `item.allowSkip`), фото с индикатором вложения (без визуального превью — CodeQL's `security-extended` XSS-запрос продолжал флагать любой динамический `<img src>` в этом файле независимо от происхождения значения (сетевой signed URL, и даже `URL.createObjectURL()` от локально выбранного файла с explicit protocol-гардом) и от способа его защиты (function-guard, inline `.startsWith`); честный компромисс — только галочка/`+`/прогресс, без `<img>` вовсе; полный просмотр уже загруженного фото остаётся у `ChecklistReviewPhotoEvidence.tsx` в очереди ревью) и прогрессом загрузки (`uploadChecklistItemPhotoWithProgress`, чей progress-callback до этого нигде не использовался), pause/resume/complete (Complete заблокирован на клиенте, пока не отвечены все обязательные критерии), одноразовая геолокация на старте/завершении (`getCurrentPosition`, никогда `watchPosition`; denied/unavailable тоже отправляются — не тихий пропуск), явный "reload"-UX на 409 (stale version) вместо generic-ошибки. Новый бэкенд-эндпоинт `PATCH /checklist-sessions/:id/feedback` (миграция `20260923160000_add_checklist_session_feedback`: `ChecklistSession.strengths/developmentAreas/nextSteps`, nullable text, плюс `feedback_updated` в `ChecklistSessionEventType`) — структурированная обратная связь оказалась не покрыта ни одним существующим полем ни в одной из PR 285-296 (честно задокументированный gap, закрытый в рамках этого PR, а не отложенный), переиспользует `checklistSessionsRun`, тот же optimistic-concurrency контракт (`version`/409), каждое поле независимо сохраняемо (partial autosave). Контекстные поля чек-листа (PR 296 `contextFields`) сознательно не реализованы в этом экране — их нет в списке критериев готовности PR 297, а для них нет backend-хранилища значений (только определения полей на самом чек-листе) — остаётся задокументированным gap для будущего PR, если/когда он потребуется. `ChecklistItemGroup` (PR 296) по-прежнему не попадает в снэпшот и не влияет на экран проведения — критерии показываются единым плоским списком, как и раньше. PR 298 (Employee: «Мои обучающие сессии») реализован: новая секция в `LearnerChecklistsPage` (без нового route/nav-item), 3 клиентских таба по статусу сессии, detail-экран с критериями; попутно закрыт PR 286-й gap — server-side enforcement `feedbackVisibility` (маскирование result/структурированной обратной связи для learner до completion при policy `after_completion`), которого раньше не было ни в одном read-пути. PR 299 (Manager: analytics API и Dashboard) реализован: единственный новый nav-item за весь workstream — `/manager/checklists`. Новый `GET /checklists/manager-analytics` (`checklistManagerAnalyticsRead`: admin/manager) — employee-first агрегация по `ChecklistSession` (каждый сотрудник в scope попадает в `employees[]`, даже с нулём сессий за период, что даёт честный `noCompletionCount`), scope — `ChecklistReviewAccessService.participantLearnerScope()` (тот же `OrganizationAccessScopeService`, не голый Group-only фильтр), низкий/средний/высокий бакеты — из `ChecklistWorkplaceSettings.highPerformanceThreshold`/`lowThreshold` (при `lowThreshold === null`, DEC-CHKS-001, честно откатывается на `passed`-флаг вместо выдуманного порога), детерминированный day-bucketed trend, per-employee тренд — сравнение текущего периода с равным по длине предыдущим. Dashboard: period-селектор (Week/Month/Quarter/custom, отражён в URL), фильтр по чек-листу, cards (sessions/completed/average/low/high/no-completion), donut (распределение) и line-chart (тренд) — оба с переключателем «график/таблица» как accessible-эквивалентом (готовых chart-примитивов в проекте не было, добавлены с нуля), таблица сотрудников с drill-down (expandable-строка со списком сессий сотрудника за период через уже существующий `GET /checklist-sessions?learnerId=...`). Фильтр по подразделению поддержан в API, но не выведен в UI в этом PR — `GET /departments` доступен только admin, а не manager (честно задокументированный, узкий gap, не блокирует остальной workstream). PR 300 (Admin: Session Report и аудируемый пересчёт) реализован: вложенный `/admin/checklists/sessions/:id` без nav-item, 5 табов, переиспользующих существующие session/instance/location/events эндпоинты; новый `POST /checklist-sessions/:id/recalculate` (admin-only, обязательная reason) пересчитывает score только из persisted `ChecklistItemResult` + immutable snapshot внутри `runSerializableWithRetry`-транзакции, всегда пишет `ChecklistScoreRevision` (даже при отсутствии изменения) плюс `score_recalculated`-событие и AuditLog-запись. PR 301 (Concurrency и idempotency) реализован: version/conditional-writes/409-на-stale-client и Serializable-транзакции с bounded retry для lifecycle/result/revision-мутаций сессии уже существовали с PR 289/291/300 — этот PR закрывает оставшийся честный пробел, идемпотентность именно `create`/`complete`/`recalculate` (reminders уже были идемпотентны на уровне БД с PR 291). Новая таблица `ChecklistIdempotencyKey` (`(organizationId, scope, key)` unique, JSONB `responseBody`); optional `idempotencyKey` на `POST /checklist-sessions`, всех `POST .../{start,pause,resume,complete,cancel}` и `POST .../recalculate` — при retry с тем же ключом внутри той же Serializable-транзакции возвращается сохранённый ответ первого вызова ещё до проверки version/status, так что легитимный повтор собственного запроса никогда не получает 409 вместо исходного успешного ответа; recalculate на replay не пишет вторую revision и не дублирует AuditLog-запись. Реальная конкурентная гонка (`Promise.all` двух одновременных `create()` с одним ключом → ровно одна сессия) проверена на настоящем Postgres. PR 302 (Observer unavailable / reassignment) реализован: orthogonal unavailable-флаг на ChecklistSession (никогда не блокирует lifecycle), новый POST .../observer-unavailable (checklistSessionsRun, только scheduled) с notification каждому admin, reassignment переиспользует существующий PATCH .../checklist-sessions/:id (checklistSessionsManage), который теперь также сбрасывает флаг при смене observerId; история — append-only ChecklistSessionEvent. PR 303 (Timezone contract) реализован: instant + IANA timezone уже хранились и reminder уже считался instant-based (PR 289/291) — закрыт реальный клиентский баг, из-за которого formatDate() форматировал scheduledAt сессии в таймзоне браузера читателя, а не в session.timezone (два зрителя в разных поясах видели разное время); wizard теперь показывает timezone только при scheduleMode='later'; DST-safety зафиксирована новыми unit- и integration-тестами на реальном переходе US DST против настоящего Postgres. PR 304 (Privacy, retention и audit) реализован: два новых `AuditLogAction` (`checklist_location.accessed`, `checklist_evidence.accessed`) пишутся после коммита транзакции для admin geo-override (submit и read) и privileged evidence-доступа вне обычного review-назначения; рутинный доступ (сам learner, назначенный reviewer) не аудируется — уже покрыт `checklist_item_result.reviewed`. Retention/deletion policy сознательно не реализована — новая `DEC-CHKS-002` в `OPEN_DECISIONS.md` фиксирует её как открытое owner-решение. PR 305 (Progressive disclosure и финальный UX) реализован: аудит routing/navigation-контракта из раздела 0.1 подтвердил, что PR 295-304 уже соблюдали контракт (ровно один checklist-пункт меню на роль, никаких лишних top-level route, scales/revisions/observer-unavailable/reminders — всё контекстно, backend-жаргон отсутствует в UI); единственный реальный gap — отсутствовавший frontend для `ChecklistWorkplaceSettings` (PR 286) — закрыт новым `ChecklistWorkplaceSettingsDialog`, contextual `Dialog` из шапки `/admin/checklists/sessions`, без нового nav-item/route; `criticalThreshold`/`lowThreshold` сознательно не показаны в форме — `DEC-CHKS-001`. PR 306 (UI State & Anomaly Matrix) частично реализован: аудит всех 17 обязательных аномалий против существующего кода (PR 285-305) показал, что большинство уже корректно покрыты предыдущими PR, а не являются пробелами этого PR — #1 (403 manager scope) на самом деле реализован как 404 (anti-enumeration: `checklist-session.service.spec.ts` — "throws 404 when the session is outside the caller scope"), формулировка плана расходится с намеренно более безопасным паттерном, реализация не меняется; #3 (409 completed in another tab) уже покрыт с обеих сторон (backend: stale-version 409 именно на `complete`-переходе, frontend: `ChecklistSessionConduct.spec.tsx` — "routes a 409 to onConflict"); #5 (observer blocked/unavailable) уже имеет frontend с PR 302/305 (badge + reassignment CTA); #12 (incomplete mandatory criteria) — не пробел, а намеренная архитектурная развязка session/instance по ADR (`ADR_CHECKLIST_SESSION_OVERLAY.md`), клиент уже блокирует Complete до ответа на все обязательные критерии; #14 (cancelled reminder suppression) протестирован в `checklist-session-reminder.worker.spec.ts`. Реально закрыто в этом PR: #8/#9 (geolocation denied/unavailable) — `captureLocationBestEffort()` путал `PERMISSION_DENIED` и `POSITION_UNAVAILABLE`, теперь различает по `GeolocationPositionError.code` и показывает разный inline-notice (`describeGeoNotice`, чистая экспортируемая функция для тестируемости) на старте/завершении; #10 (required location + audited override) — backend admin-override уже существовал с PR 290/304, но не имел frontend; добавлен `LocationOverrideForm` в admin session report (Files tab), с обязательной причиной, аудируемой через `checklist_location.accessed`; #2 (404 session) — существующий `notFound`-вариант `AsyncDataState` довёрстан в `ChecklistSessionConduct` и `AdminChecklistSessionReportPage`. На момент мержа этого PR честно оставались открытыми 8 пробелов (#4, #6, #7, #11, #13, #15, #16, #17) — все они закрыты последующим follow-up PR, см. ниже "PR 306-followup". PR 307 (E2E, security, accessibility, visual regression) реализован: новый `checklist-session-lifecycle.spec.ts` — 5 функциональных E2E-тестов на реальном браузере/frontend с реальным login/RBAC (checklist-session API per-test route-mocked, как в существующем `checklist-review-workflow.spec.ts`; реальное backend-покрытие — отдельные DB-integration/unit-тесты), закрывающих 14 из 15 обязательных сценариев (#13 намеренно не в E2E — backend-only поведение без наблюдаемого UI-сигнала, уже покрыто worker-тестом); в процессе найдены и исправлены два реальных production-бага (wizard и observer conduct screen теряли локальный UI-стейт из-за `useAsyncData`'s `reload()` flip в `'loading'` на каждую мутацию — заменено на `mutate()`); cross-tenant/IDOR matrix дополнен checklist-session записями; accessibility — 0 нарушений на всех проверенных экранах; visual regression — 0 диффов на изменённых этим PR экранах. PR 308 (Документация, release readiness, production verification gates) реализован: API/RBAC-документация синхронизирована (одна недостающая запись — PR 302 — плюс post-merge scored/idempotency фикс #797/#798), новый cross-role `CHECKLIST_SESSION_WORKFLOW.md`, migration/rollback-секция для единственной data-dependent миграции модуля; changelog/release-note — подтверждено отсутствие такой конвенции в репозитории (не изобретена новая); production verification gates проверены против реального Railway-деплоя, а не оставлены неизвестными — **Redis/background worker и mail provider подтверждены LIVE-VERIFIED и оказались не сконфигурированы в production** (все recurring-джобы, включая уже существующий `ChecklistDeadlineWorker`, сейчас неактивны), остальные gates либо LIVE-VERIFIED-положительны (object storage сервис), либо уже задокументированы как открытое owner-решение (geolocation retention, DEC-CHKS-002) или deferred (load/latency), либо остаются `[НЕ ПРОВЕРЕНО]` там, где живая проверка недоступна из sandbox-окружения (bucket CORS, alerting-платформа).

**PR 306-followup (закрытие оставшихся 8 аномалий)** реализован: #17 (duplicate create/complete) — `idempotencyKey` (backend, PR 301) теперь реально отправляется с фронтенда: `transitionChecklistSession()`/`recalculateChecklistSessionScore()` принимают опциональный `idempotencyKey`, каждый вызывающий код (`ChecklistSessionConduct`'s start/pause/resume/complete, `AdminChecklistSessionsPage`'s cancel, wizard'а auto-start, `RecalculateForm`) генерирует свежий `crypto.randomUUID()` на каждое действие. #11 (all-skipped vs not-yet-scored) — новый чистый хелпер `describeChecklistSessionResult()` (`shared/checklistStatus.ts`) различает `pending`/`notScored`/`scored` по `instanceStatus`, довёрстан во все 4 места, где результат сессии показывается пользователю (admin sessions list, admin session report, manager dashboard, employee list+detail), с новым `notScored`-текстом в i18n вместо неотличимого от "ещё не завершено" пустого `—`. #4 (checklist archived during wizard) — новый DB-integration тест (`checklist-session-admin-api.database.spec.ts`) на настоящем Postgres подтверждает, что архивация чек-листа между загрузкой wizard'а и submit'ом абортит весь batch без частичных строк (уже была правильная бизнес-логика, не хватало проверки против реальной БД, а не мока). #6/#7 (photo rejected/storage failure) — новый `checklists.photo-upload.spec.ts` (0 тестов на `uploadItemPhoto` раньше) покрывает: отклонение неразрешённого MIME-типа до вызова upload-сервиса, propagation сбоя storage-слоя вместо тихого проглатывания, удаление orphan-объекта при падении `attachItemPhoto` после успешного upload (и что и в этом случае исходная ошибка не теряется, даже если сам cleanup тоже падает). #13 (reminder retry) — `checklist-session-reminder.worker.spec.ts` теперь доказывает, что сбой `ChecklistSessionReminderDelivery.send()` пробрасывается наружу (не глушится), давая queue-level retry `BackgroundJobsService` реальный шанс сработать, и что повторная попытка после сбоя безопасна (`deliverEmail` не хранит собственного состояния). #15 (analytics no-data) — новый DB-integration тест на реальном Postgres (`checklist-manager-analytics.database.spec.ts`) проверяет честный zero-row для сотрудника без единой сессии (`noCompletionCount`, `averagePercentage: null`, пустой `distribution`/`trend`), плюс новый frontend-тест на явное сообщение "нет сотрудников в зоне ответственности" вместо пустой таблицы. #16 (partial chart data) — новый frontend-тест на смешанную полноту данных в таблице сотрудников (строка с нулём сессий рядом со строкой с реальным результатом рендерится без `null`/`undefined` в DOM). Все 8 пробелов закрыты тестами на реальных данных (Postgres или полный frontend render), не только исправлением логики.
...
(`ChecklistItemSummary.scale` добавлено этим PR). Фото: без визуального `<img>`-превью (CodeQL's
`security-extended` XSS-запрос флагал любой динамический `<img src>` в этом файле вне зависимости
от происхождения/защиты значения — задокументированный gap, см. ниже), только индикатор
вложения/прогресс через `uploadChecklistItemPhotoWithProgress`
...
для будущего PR (закрывается PR 323 в `UI_REFRESH_IMPLEMENTATION_PLAN.md`). `ChecklistItemGroup` (PR 296) по-прежнему не в снэпшоте и не влияет на этот экран.
...
**Что сделано:** новая секция «Мои обучающие сессии» дописана в конец существующей
`LearnerChecklistsPage` (`LearnerChecklistSessions`/`LearnerSessionDetail`, новые файлы), новый
route/nav-item не создан. Переиспользует `GET /checklist-sessions` без доп. фильтра
(`sessionScope()` для чистого `learner` уже `{instance:{userId}}`); 3 клиентских таба
(Назначено/В процессе/Завершено, маппинг `scheduled→Назначено`, `in_progress`+`paused→В процессе`,
`completed`+`cancelled→Завершено`). Detail-экран дополнительно подгружает `GET
/checklist-instances/:id` для read-only списка критериев с комментариями наблюдателя.
Попутно закрыт реальный gap: PR 286 ввёл `ChecklistWorkplaceSettings.feedbackVisibility`
(`after_completion`/`live`), но ни один read-путь никогда её не применял — этот экран первый, кому
это действительно нужно (иначе учащийся видел бы счёт/структурированную обратную связь PR 297 до
завершения сессии). `GET /checklist-sessions`/`GET /checklist-sessions/:id` теперь маскируют
`result` (percentage/passed/scored → null, новый флаг `visible: false`) и
strengths/developmentAreas/nextSteps для чистого learner, пока policy `after_completion` и
`ChecklistInstance.status !== 'completed'`; admin/manager/instructor никогда не маскируются, `live`
никогда не маскирует. Значения обнуляются по-настоящему (не просто флаг), значит несанкционированное
число не попадает в тело ответа. Новых role policies нет.

**Критерии готовности:**
- [x] реализовано внутри `/learn/checklists`, новый nav-item не создан;
- [x] employee видит только свои sessions; status/result понятны; completed открывает result;
- [x] admin-only данные отсутствуют (feedbackVisibility enforcement — см. выше).
...
**Критерии готовности:**
- [x] `/manager/checklists` — единственный новый пункт меню всего плана;
- [x] scope применяется ко всем aggregates через `OrganizationAccessScopeService`, не голый Group-only `ManagerTeamScope`;
- [x] число sessions не пере-взвешивает employee; no-completion не занижает average; trend/distribution детерминированы;
- [x] metrics только из scoped API; filters отражены в URL; charts имеют accessible text equivalent;
- [x] visual regression test есть; aggregation tests проходят.

**Что сделано:** `GET /checklists/manager-analytics` (`ChecklistsService.getManagerAnalytics`) — employee-first агрегация по `ChecklistSession`, scope через `ChecklistReviewAccessService.participantLearnerScope()`. `/manager/checklists` (`ManagerChecklistsPage`) — period/checklist фильтры (в URL), StatsGrid cards, donut (распределение), line chart (тренд), employee `DataTable` с expandable drill-down в сессии сотрудника. Оба графика реализованы с нуля (готовых chart-примитивов в проекте не было) и снабжены переключателем «график/таблица» как accessible text equivalent. Департаментный фильтр поддержан в API, но не выведен в UI (department-листинг — admin-only эндпоинт).
...
**Критерии готовности:**
- [x] вложенный экран без отдельного nav-item; данные соответствуют snapshot/history;
- [x] nested resources object-scoped; tabs имеют loading/error/empty; history immutable client-side;
- [x] старый результат сохранён при пересчёте; actor/reason/before/after сохранены; non-admin denied;
- [x] UI показывает revisions.

**Что сделано:** `/admin/checklists/sessions/:id` (`AdminChecklistSessionReportPage`) — вложенный drill-down (новая ссылка «Отчёт» в списке сессий, без nav-item), 5 табов (Сводка/Участники/Критерии/Файлы/История), переиспользует существующие `GET /checklist-sessions/:id`, `GET /checklist-instances/:id` (включая `ChecklistReviewPhotoEvidence` для фото), `GET /checklist-sessions/:id/location`, `GET /checklist-sessions/:id/events`. Кнопка «Печать» (`window.print()`) — вся print-friendly часть, серверный PDF/export не запрашивался. Новый `POST /checklist-sessions/:id/recalculate` (`checklistScoreRecalculate`: admin-only, обязательная непустая `reason`) — `ChecklistsService.computeInstanceScore()` пересчитывает только из persisted `ChecklistItemResult` + immutable `templateSnapshot`, никогда не трогает статус/результаты (в отличие от `recomputeInstance()`); `algorithm dispatch по version` не понадобился — сегодня существует только snapshot version 1, `getStoredSnapshot()` уже отклоняет неизвестную версию defensively. Пересчёт и запись `ChecklistScoreRevision` идут в одной `runSerializableWithRetry`-транзакции (retry/race protection); revision пишется всегда, даже если пересчёт не изменил число (сам факт ручной проверки — часть аудита); плюс `score_recalculated` `ChecklistSessionEvent` и `AuditLogService`-запись. `GET /checklist-sessions/:id/score-revisions` отдаёт историю. Пункт «Repeat session» из Report bullet-списка не дублируется на экране отчёта — действие уже существует на списке сессий (PR 292). Честно задокументированный узкий gap: accessibility-тест с реальным логином для этого конкретного экрана не добавлен — в demo-окружении нет засеянных `ChecklistSession`, а создание сессии через полный wizard из a11y-теста расценено как не стоящее риска flakiness без живого прогона для проверки селекторов; экран покрыт unit-тестами (loading/summary/participants/criteria/history/recalculate), integration-тестом на реальном Postgres и mocked visual-regression тестом.
...
**Что сделано:** `version`/conditional-writes/409, Serializable-транзакции с bounded retry (`runSerializableWithRetry`, `MAX_SERIALIZATION_RETRIES = 5`, retry только на P2034) и DB-уровневая идемпотентность reminders уже существовали до этого PR (PR 289/291/300) — этот PR добавил недостающую идемпотентность для `create`/`complete`(и остальных transition-действий)/`recalculate` через новую таблицу `ChecklistIdempotencyKey` (`(organizationId, scope, key)` unique, JSONB `responseBody`), optional `idempotencyKey` в теле запроса, check-before-version-check внутри той же Serializable-транзакции. Юнит-тесты (67 в `checklist-session.service.spec.ts`, +6 для идемпотентности) и real-Postgres интеграционные тесты (`checklist-session-idempotency.database.spec.ts`, 4 теста, включая конкурентную гонку `Promise.all`) проверяют оба сценария: последовательный retry и настоящий concurrent double-send.

**Критерии готовности:**
- [x] concurrent complete создаёт один terminal result (version+status conditional update, PR 289; retry с тем же idempotencyKey никогда не создаёт второе событие/мутацию, PR 301);
- [x] retry не создаёт дубликаты (idempotencyKey replay для create/transition/recalculate — PR 301; DB-conditional claim для reminders — PR 291);
- [x] stale client получает 409 без silent overwrite (version mismatch/updateMany count check, PR 289 — не затронуто этим PR, кроме порядка проверки: idempotency-key-hit проверяется раньше, чтобы легитимный собственный retry не получал 409).
...
**Что сделано:** `ChecklistSession.observerUnavailableReason`/`observerUnavailableAt` — orthogonal business-флаг (не lifecycle status), никогда не блокирует start/pause/resume/complete/cancel. Новый `POST /checklist-sessions/:id/observer-unavailable` (`checklistSessionsRun`: admin/instructor, только пока сессия `scheduled`, тот же version/409-контракт) пишет `observer_marked_unavailable` `ChecklistSessionEvent` и создаёт in-app `Notification` каждому admin организации (менеджеры не таргетируются индивидуально — честно задокументированный gap: обратный расчёт "какие менеджеры видят этого сотрудника" не выведен `OrganizationAccessScopeService`). Reassignment переиспользует уже существующий `PATCH /checklist-sessions/:id` (`checklistSessionsManage`: admin/manager, `sessionScope()`), который теперь дополнительно сбрасывает unavailable-флаг при смене observerId — вся история (unavailable + reassign) сохраняется как append-only `ChecklistSessionEvent`, а не тихой перезаписью. Frontend: кнопка «Can't conduct this» на `ChecklistSessionsToConduct` (список наблюдателя) для ещё не начавшихся сессий; на `AdminChecklistSessionsPage` — бейдж «Unavailable» в колонке Observer и действие «Reassign observer», открывающее `ReassignObserverDialog` (переиспользует `WizardDialog` как single-step shell, без третьей реализации модалки, и тот же debounced observer-picklist, что и `ChecklistSessionWizard`).

**Post-merge review fixes** (Codex-ревью пришло уже после мержа PR 302, исправлено отдельным follow-up PR): (1) `POST .../observer-unavailable` теперь использует новый `observerActionScope()` вместо read-ориентированного `sessionScope()` — для пользователя с ролями manager+instructor `sessionScope()` намеренно возвращает union (команда ИЛИ personally observed), что верно для чтения, но позволяло такому пользователю отметить недоступным наблюдателя в чужой сессии команды; (2) reassignment на того же самого наблюдателя теперь отклоняется (`BadRequestException`) вместо тихого сброса валидного unavailable-флага без назначения замены, плюс текущий наблюдатель исключён из пиклиста `ReassignObserverDialog`; (3) диалог «Can't conduct this» сбрасывает `reason`/`error` при каждом открытии (`useEffect` по `session?.id`) — раньше при повторном открытии для другой сессии могла остаться старая причина; (4) `NotificationBell` теперь реально подключён в `AdminPageLayout` (раньше был статичный `🔔`-span) — admin, основной получатель уведомления `checklist_session_observer_unavailable`, никак не видел его в интерфейсе; добавлены недостающие `notifications.types.*` i18n-строки для этого типа и двух его PR 291-родственников (`checklist_session_pre_start_reminder`, `checklist_session_incomplete_after_start_reminder`) во всех 4 локалях.

**Критерии готовности:**
- [x] unavailable не блокирует управление session (флаг orthogonal статусу, transition()/submitFeedback() не проверяют его вовсе — подтверждено integration-тестом: session стартует сразу после markObserverUnavailable);
- [x] reassignment object-scoped (переиспользует существующий `update()`'s `sessionScope()` — ничего нового не потребовалось);
- [x] история назначения сохраняется (append-only `ChecklistSessionEvent`: `observer_marked_unavailable` → `rescheduled`/`observer_reassigned`, проверено integration-тестом).
...
**Что сделано:** хранение instant (`ChecklistSession.scheduledAt`, timestamptz) + IANA timezone (`ChecklistSession.timezone`) и instant-based расчёт reminder (`scheduledFor = scheduledAt.getTime() - LEAD_MS`, чистая миллисекундная арифметика, без local-calendar расчётов) уже существовали с PR 289/291 — честно не были покрыты явными DST-тестами и, что важнее, читающий UI игнорировал `session.timezone` и форматировал `scheduledAt` в **таймзоне браузера читателя**, а не в таймзоне самой сессии — то есть два разных зрителя одной сессии в разных часовых поясах видели разное время. Этот PR закрывает оба пробела: (1) все места, показывающие `scheduledAt` сессии (`AdminChecklistSessionsPage`, `AdminChecklistSessionReportPage`, `ManagerChecklistsPage`, `LearnerChecklistSessions`, `ChecklistSessionsToConduct`) теперь передают `timeZone: session.timezone` в `formatDate()`/`Intl.DateTimeFormat` — event/audit-таймстампы (`ChecklistSessionEvent.createdAt`, revisions, location captures) сознательно оставлены в таймзоне зрителя, так как это "когда я это увидел", а не часть расписания; (2) wizard показывает строку "Часовой пояс: …" только при `scheduleMode === 'later'` (при "Начать сейчас" уточнять нечего); (3) новые юнит- и integration-тесты явно фиксируют instant-based DST-safety `upsertPreStartReminder`/`createIncompleteAfterStartReminder` (включая реальный переход через весенний перевод стрелок US DST 2026-03-08 против настоящего Postgres) — не изменение поведения, а регрессионный якорь для уже правильной логики.

**Критерии готовности:**
- [x] UI/API показывают одинаковое время (всё, что относится к расписанию сессии, форматируется в `session.timezone`, не в локальной таймзоне зрителя — API уже отдавал единственный UTC-instant, проблема была только на клиенте);
...
**Критерии готовности:**
- [x] ежедневный admin flow начинается с `/admin/checklists`;
- [x] primary nav не сложнее исходного `admin.nav.checklists` + один пункт `/manager/checklists`;
- [x] backend jargon отсутствует в основном UI.

**Что сделано:** полный аудит routing/navigation-контракта из раздела 0.1 против production-кода
(nav-defs, route-файлы, все экраны module) подтвердил, что PR 295-304 уже соблюдали контракт:
ровно один checklist-пункт меню на роль (`/admin/checklists`, `/instructor/checklists`,
`/learn/checklists`, `/manager/checklists`), ни одного лишнего top-level route (session report,
observation-sheet builder, observer conduct — всё под-маршруты/вкладки/in-page state внутри
существующих точек входа, местами даже более консолидированно, чем сама таблица маршрутов
предполагала), scales — исключительно `ScaleManagerDialog` из builder, revisions — исключительно
history-таб `AdminChecklistSessionReportPage`, observer unavailable — badge + reassignment-CTA в
`AdminChecklistSessionsPage`/`ReassignObserverDialog`, reminders — обычные записи в
`NotificationBell` без отдельного diagnostics-экрана. Grep по backend-жаргону
(`ManagerTeamScope`/`OrganizationAccessScopeService`/`snapshotVersion`/`idempotencyKey`/`P2034`/
`denominator`/`worker`) по всему `apps/web/src` — ноль совпадений.
Единственный реальный gap: `ChecklistWorkplaceSettings` (backend, PR 286) не имел frontend UI
вообще — ни диалога, ни API-клиента. Закрыт в этом PR: `ChecklistWorkplaceSettingsDialog` (contextual
`Dialog`, как `ScaleManagerDialog`, открывается кнопкой «Настройки» в шапке `/admin/checklists/sessions`,
не новый nav-item/route) редактирует `moduleEnabled`/`highPerformanceThreshold`/
`defaultGeolocationPolicy`/`feedbackVisibility`. `criticalThreshold`/`lowThreshold` в этой форме
...
**Критерии готовности:**
- [x] critical E2E зелёные; cross-tenant/IDOR matrix зелёная (новые `checklist session` записи в `cross-tenant-idor.audit.spec.ts`);
- [x] existing Checklist tests зелёные (2307 backend unit, 145 DB-integration, 798 frontend unit — все прошли без регрессий);
- [x] mobile observer baseline зелёный; admin/manager desktop baselines зелёные (accessibility: 0 нарушений; visual regression: 0 диффов на изменённых экранах);
  (Зафиксировано 2026-09-25 по итогам зелёного CI-прогона PR #797, GitHub Actions `ci.yml`, ubuntu-latest — эти цифры относятся к тому прогону, не к произвольному будущему состоянию ветки.)
- [x] CI не ослаблен (никаких изменений в конфигурации CI/gates).

**Что сделано:** новый `apps/e2e/tests/checklist-session-lifecycle.spec.ts` — 5 функциональных E2E-тестов
на реальном браузере/frontend с реальным login/RBAC (checklist-session API per-test route-mocked,
как и в существующем `checklist-review-workflow.spec.ts` — не полностью реальный backend; реальное
backend-покрытие домена — отдельные DB-integration/unit-тесты: `checklist-session-recalculate.database.spec.ts`,
`checklist-session-idempotency.database.spec.ts`, `checklist-session.service.spec.ts`,
`cross-tenant-idor.audit.spec.ts`), покрывающих 14 из 15 обязательных сценариев: полный
жизненный цикл (admin создаёт сессию через wizard → появляется в list → observer проводит на мобильном
375px viewport с реальным `context.setGeolocation()` на старте/завершении → прикрепляет обязательное
фото через `setInputFiles` → employee видит результат → admin открывает отчёт → пересчитывает балл,
создавая новую revision) закрывает #1,#2,#3,#4,#5,#6,#11,#12; отдельный тест на manager-аналитику
закрывает #7,#8 и частично #9 (UI-faithfulness к тому, что вернул scoped API-ответ — реальное
cross-tenant/scope-enforcement уже проверено на настоящем Postgres в
`checklist-manager-analytics.database.spec.ts`); отдельные тесты — #10 (снэпшот критерия стабилен
после редактирования live-чек-листа), #14 (409 на гонке двух complete-запросов даёт safe reload-prompt,
не silent overwrite), #15 (all-skipped даёт явный "не оценено", не пустой результат). #13 (cancelled
session no reminder) намеренно не в E2E — это backend-only поведение без наблюдаемого UI-сигнала,
уже покрыто `checklist-session-reminder.worker.spec.ts`'s suppression-тестом.

Попутно найдены и исправлены **два реальных production-бага**, обнаруженных только благодаря
E2E-тестам на настоящем стеке (моки бы их не поймали): (1) в `AdminChecklistSessionsPage` фоновый
`load()` после создания сессии в визарде мгновенно flips `AsyncDataState` в `'loading'`, а
компонент на этом статусе full-page-early-return'ит — визард (со своим "Сессии созданы" экраном)
полностью размонтируется и перемонтируется заново на шаге 0, так что пользователь никогда не видит
подтверждение; (2) тот же паттерн, но серьёзнее, в `ChecklistSessionConduct` — КАЖДОЕ действие
наблюдателя (отметить критерий, приложить фото, изменить структурированную обратную связь, любой
lifecycle-переход) вызывало `onReload()`, размонтируя весь экран проведения и сбрасывая `stepIndex`
обратно на первый критерий; тот же паттерн — в `AdminChecklistSessionReportPage` при пересчёте балла
(сбрасывался выбранный таб). Все три исправлены одним и тем же способом: `useAsyncData`'s `mutate()`
(уже существовал, просто не использовался) мержит ответ мутации в локальный стейт напрямую, без
полного network-reload и связанного с ним flip в `'loading'` — `useAsyncData` сам не тронут (шире
используемая инфраструктура, вне блast radius этого PR). Cross-tenant/IDOR matrix
(`cross-tenant-idor.audit.spec.ts`) дополнен двумя записями для `ChecklistSessionService.get()`/
`listEvents()` — раньше checklist-эндпоинты в этом файле не проверялись вовсе (хотя эквивалентная
org-scoping логика уже была покрыта в `checklist-session.service.spec.ts`, отдельная явная запись в
общем IDOR-матриксе была честным пробелом). Accessibility (`admin/checklists/sessions` + wizard,
`instructor` mobile "Проведение", `learn/checklists`, `manager/checklists`) — 0 нарушений WCAG AA.
Visual regression — 0 диффов на `admin-checklist-session-report`/`instructor-checklist-session-conduct`
(единственные экраны, чью разметку затронул этот PR); диффы в неизменённых экранах (`admin-checklist-builder`,
`admin-checklist-sessions-wizard`, `manager-checklist-analytics`) в локальной среде — задокументированный
в `visual-tests/responsive-matrix.spec.ts` артефакт версии Chromium (1px ширины/scrollbar, не layout-регрессия),
подтверждается финальным CI-прогоном на закреплённой версии браузера.
...
**Критерии готовности:**
- [x] docs соответствуют runtime; не описаны несуществующие функции;
- [x] API/RBAC/security синхронизированы; rollback strategy описана;
- [x] каждый production-gate подтверждён фактической проверкой, неподтверждённое — `[НЕ ПРОВЕРЕНО]`;
- [ ] release checklist завершён — `docs/runbooks/RELEASE_GATE.md`'s release-evidence JSON (`pnpm release:gate`) is produced by the release owner at actual deploy time for a specific SHA, not fabricable from a documentation PR; CI/docs-consistency prerequisites for it are green as of this PR.

**Что сделано:** API/RBAC-документация синхронизирована с runtime, а не переписана заново — `docs/contracts/API_RBAC_MATRIX.md` уже покрывал PR 289-300, добавлена недостающая запись PR 302 (`observer-unavailable`, отдельный `observerActionScope()` вместо read-oriented `sessionScope()` — предотвращает privilege leak для dual-role manager+instructor); `docs/contracts/API_CONTRACTS.md` уже покрывал PR 289-307 практически полностью, добавлен только post-merge фикс #797/#798 (`scored`/`newScored` idempotency-баг), которого там не было. Вместо 4 отдельных "guides"-файлов (в таксономии `docs/README.md` для такого формата нет прецедента ни у одного другого модуля) — один новый contract-документ, `docs/contracts/CHECKLIST_SESSION_WORKFLOW.md`, организующий уже существующую детальную документацию по ролям (admin/manager/observer/learner), а не дублирующий её. Migration/rollback: добавлена секция в `docs/runbooks/MIGRATION_BACKUP_POLICY.md` для единственной data-dependent миграции модуля (`20260925120000_add_checklist_score_revision_scored`) — backfill проверен на реальных данных в откатываемой транзакции, а не только тестами. Changelog/release note: в репозитории нет ни одного файла или конвенции changelog/release-note (проверено — не только для checklist-модуля, ни для одного из ~20 других завершённых модулей), поэтому вместо изобретения новой, нигде больше не используемой категории документации этот раздел плана (все "Что сделано"-абзацы PR 285-308) остаётся тем, чем он де-факто уже был весь этот workstream.

Production verification gates (`docs/quality/READINESS_AND_SECURITY_GATES.md`, новая секция) — каждый gate подтверждён фактической проверкой живого Railway-деплоя (`serg3004/lms-for-my-using`, `reasonable-reprieve`/production, 2026-09-25), а не оставлен "неизвестным": **Redis/background worker — LIVE-VERIFIED, и подтверждение отрицательное.** В production `api` нет `REDIS_URL` вовсе (приложение стартовало только благодаря `ALLOW_IN_MEMORY_RATE_LIMIT`); `BackgroundJobsModule`'s provider безусловно падает на `DisabledBackgroundJobBackend` без `REDIS_URL` — это значит, что **все** recurring-джобы, включая уже существующий `ChecklistDeadlineWorker` и новый `ChecklistSessionReminderWorker` (PR 291), сейчас неактивны в production (`upsertRecurring()` молча no-op, `enqueue()` бросает 503). Предположение из формулировки этого PR ("уже используется — подтвердить") не подтвердилось для реального деплоя. Отдельно и независимо не сконфигурирован ни один из delivery-webhook (`CHECKLIST_SESSION_REMINDER_DELIVERY_URL`, `PASSWORD_RESET_DELIVERY_URL`) — mail provider/SLA gate тоже LIVE-VERIFIED и тоже отрицательный. Object storage — частично LIVE-VERIFIED: `minio` живой с персистентным volume, `S3_*` переменные согласованы между `api`/`malware-scanner`, но CORS-политика самого bucket и живой presigned upload/download smoke-test остаются `[НЕ ПРОВЕРЕНО]` — исходящий HTTP из этой sandbox-среды заблокирован прокси. Geolocation retention — не пробел этого PR, уже открытое owner-решение `DEC-CHKS-002`. Production-like load/latency — уже задокументирован как deferred, не obligation. Observability — метрика `lms_checklist_session_reminder_delivery_errors_total` существует и подключена к общему `/metrics`, но подключение к реальному alerting (Sentry/PagerDuty) остаётся `[НЕ ПРОВЕРЕНО]` — недоступно без доступа к самой alerting-платформе. Redis/mail-провайдер — не описываются как гипотетический риск: это подтверждённое текущее состояние живого деплоя, требующее отдельного owner-решения (провижининг Redis — инфраструктурное изменение вне scope документационного PR).
...
- [x] существует admin wizard с 4 шагами (модалка внутри `/admin/checklists`);
- [x] существует admin session list с tabs/search/filters/statuses (под `/admin/checklists`);
- [x] существует observation sheet builder (под `/admin/checklists`);
- [x] существует полноценный mobile observer flow (внутри `/instructor/checklists`);
- [x] существует employee «Мои обучающие сессии» (внутри `/learn/checklists`);
- [x] существует manager dashboard (`/manager/checklists` — единственный новый пункт меню всего плана);
- [x] dashboard содержит average/distribution и trend visualization; employee table и drill-down;
- [x] существует session report с tabs (вложенный экран, без своего nav-item);
- [x] lifecycle `ChecklistSession` соответствует backend state machine и согласован с `ChecklistInstance.status`;
- [x] scale/weight/groups/context fields/structured feedback работают по snapshot;
- [x] photo (через существующий `ChecklistItemResult`)/geolocation/comments/Skip работают по policy;
- [x] reminders используют persistent ledger и реально исполняются, не мешая существующему `ChecklistDeadlineWorker` (code/test-уровень: DB-idempotent ledger, worker логика проверена unit/integration-тестами против реального Postgres; live-исполнение в текущем production-деплое зависит от Redis, который там не сконфигурирован — см. следующий пункт и `docs/quality/READINESS_AND_SECURITY_GATES.md`'s PR 308 секцию);
- [x] observer unavailable/reassignment работает;
- [x] recalculation сохраняет immutable history;
- [x] RBAC/object-level enforced backend-ом через `OrganizationAccessScopeService`;
- [x] loading/empty/error/403/404/409/photo/location states реализованы;
- [x] desktop admin/manager визуально сопоставимы с прототипом; mobile observer визуально сопоставим с прототипом;
- [x] существующий LMS design system сохранён; accessibility и visual regression проходят;
- [x] существующие Checklist flows (unit/integration/E2E) не регрессировали;
- [x] обязательный CI зелёный;
- [ ] production verification gates подтверждены перед rollout — подтверждены фактической проверкой (не оставлены неизвестными), но **не все положительны**: Redis/background worker и mail provider/delivery webhook подтверждены отсутствующими в текущем live production-деплое (`docs/quality/READINESS_AND_SECURITY_GATES.md`'s PR 308 секция) — reminders (существующий `ChecklistDeadlineWorker` и новый `ChecklistSessionReminderWorker`) сейчас не исполняются live, независимо от корректности их кода. Это открытый owner-blocker для полного rollout notification-зависимой части модуля, не пробел документации;
- [x] за весь модуль появился ровно один новый top-level пункт меню (`/manager/checklists`) — если появилось больше, это регрессия к ошибке модуля «Оргструктура» и должно быть исправлено до мержа.
