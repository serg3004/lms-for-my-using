...
**Edge cases:**
- `slug` уникален глобально (не в рамках чего-либо) — коллизия при регистрации новой организации требует явной обработки, а не generic 500.
- Soft-delete (`deletedAt`) организации не каскадирует автоматически на связанные записи на уровне БД (Prisma `onDelete` работает только при физическом удалении) — soft-delete родителя должен явно учитываться в every query, иначе «удалённая» организация продолжит быть видна через дочерние записи.
- `plan` — enum без реального биллинга за ним (см. отраслевую карту, Subscription/Invoice/Payment, п. 11.1 нереализованных) — апгрейд/даунгрейд плана сейчас чисто административное действие без финансовых последствий.
...
**Edge cases:**
- `onDelete: Restrict` на связи с `Organization` — нельзя физически удалить организацию, пока в ней есть пользователи; это защищает от случайного каскадного удаления, но означает, что удаление организации требует явного порядка операций.
- Один и тот же email может существовать в разных организациях как разные пользователи (уникальность per-organization) — при переходе сотрудника между организациями-клиентами это два независимых аккаунта, не перенос одного.
- `status: invited` — приглашённый, но ещё не активировавшийся пользователь; не описан явный TTL приглашения в схеме — «зависшие» приглашения не самоочищаются автоматически.
...
**Edge cases:**
- Архивация группы (`status: archived`) не описывает явно, что происходит с активными `Assignment`, назначенными на всю группу (`Assignment.groupId`) — назначения могут «зависнуть» на архивной группе.
- `slug` уникален в рамках организации — переименование группы с изменением slug может конфликтовать с уже использованными ссылками/интеграциями, если slug используется во внешних URL.
...
**Edge cases:**
- Составной PK `[groupId, userId]` не позволяет хранить историю повторного вступления/выхода (soft-delete через `deletedAt` — но при повторном добавлении после `deletedAt` нужен `upsert`, а не plain `create`, иначе конфликт уникальности PK).
- Каскадное удаление при физическом удалении пользователя `onDelete: Cascade` — если когда-либо потребуется физическое (не soft) удаление пользователя, членства в группах исчезнут без следа, в отличие от soft-delete подхода остальной схемы.
...
**Edge cases:**
- Менеджер закрепляется за группой, а не строит организационную иерархию (в отличие от отсутствующей в проекте `Manager Hierarchy`, п. 1.4 нереализованных) — менеджер видит только группы, где явно закреплён, не «свою вертикаль» произвольной глубины.
- Один менеджер может быть закреплён за несколькими группами одновременно — агрегация «моя команда» должна объединять учеников по всем группам менеджера, не только по последней.
...
**Edge cases:**
- ~~`status: pending`, не завершённый до `expiresAt` — модель не содержит триггера автоматического перевода в `aborted`; нужен периодический job~~ — **частично закрыто (2026-08-08):** добавлен `.github/workflows/multipart-upload-cleanup.yml` — ежедневный (03:00 UTC) прогон уже существовавшего `multipart-upload-cleanup.ts` в dry-run режиме (только отчёт, без реального аборта — это безопасно и работает уже сейчас). Реальный `--execute` (абортит через S3/R2) доступен только вручную через `workflow_dispatch`, так как зависит от следующего пункта — S3/R2 всё ещё не настроен. Workflow требует секреты `PRODUCTION_DATABASE_URL`/`PRODUCTION_JWT_SECRET` в GitHub — без них видимо и явно падает на шаге отчёта, не молчит.
...
4. **RBAC-документ и `roles.spec.ts` — best-effort зеркала кода, не источник истины.** Сам `docs/API_RBAC_MATRIX.md` explicitно предупреждает: единственное, что реально проверяет CI — `apps/api/src/modules/auth/roles.ts`. При реализации любой из 99 нереализованных сущностей это тот же риск дрейфа, что уже подтверждён на живых примерах (`themeSettingsRead`, `managerTeamSummaryRead`).
