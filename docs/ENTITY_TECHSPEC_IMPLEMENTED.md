...
**Edge cases:** проверены (2026-08-08).
- **Важное структурное наблюдение, подтверждено:** `AssessmentAttemptStatus` enum содержит только одно значение — `completed`. Модель на уровне схемы не различает «попытка в процессе» и «попытка завершена» — API создаёт запись только по факту полной сдачи, `startedAt` фактически совпадает с `completedAt` (оба выставляются в один и тот же момент вызова `createAttempt`, промежуточного состояния нет). Прямо связано с добавляемым ниже ограничением по времени (`timeLimitMinutes`, отдельная задача) — оно и стало тем самым триггером, который требует нового статуса `in_progress`.
