...
- ~~`AssessmentAttemptStatus` enum содержит только одно значение — `completed`, нет промежуточного состояния~~ — **закрыто (2026-08-08), реализовано по решению пользователя.** Добавлен `timeLimitMinutes Int?` на `Assessment` (nullable = без лимита) и статус `in_progress`. Для тестов **без** лимита времени (подавляющее большинство, поведение по умолчанию не меняется) всё работает как раньше — один вызов `POST /assessments/:id/attempts` сразу создаёт `completed`-попытку. Для тестов **с** лимитом:
  ...
  - ~~Брошенная `in_progress`-попытка навсегда блокирует удаление `Assessment` (`DELETE /assessments/:id`, §15) — авто-финализации/expiry нет, только прямая правка БД~~ — **закрыто (2026-08-09), по решению пользователя.** `AssessmentsService.deleteAssessment` теперь считает `in_progress`-попытку блокирующей удаление, только если она ещё не вышла за `timeLimitMinutes` (`startedAt > now − timeLimitMinutes`) — та же логика, что уже применяется к просроченной отправке (см. выше). Если лимит времени у теста впоследствии убрали (`timeLimitMinutes = null`), проверка на истечение не работает — любая `in_progress`-попытка по-прежнему блокирует удаление (нет базы для расчёта просрочки). Слот в `maxAttempts` при этом всё ещё не освобождается (см. пункт выше) — решается отдельно, если понадобится.
...
**Edge cases:** проверены (2026-08-08).
- ~~Два разных механизма хранения выбора — схема сама не enforce-ит консистентность~~ — **закрыто, при перепроверке не подтвердилось.** Двойная защита: `createAssessmentAttemptAnswerSchema.refine()` требует ровно одно из `selectedOptionId`/`selectedOptionIds` на уровне Zod, а `gradeSingleChoiceAnswer`/`gradeMultipleChoiceAnswer` дополнительно проверяют соответствие использованного поля реальному `question.type` (иначе `BadRequestException`). `AssessmentResultsService` прокидывает оба поля как есть, не переинтерпретируя по текущему `type` — риска рассинхронизации при редактировании вопроса (теперь возможном через `PATCH /questions/:id`, §16) нет: старые ответы не перечитываются под текущий тип задним числом.
- ~~`onDelete: Restrict` на `questionId`~~ — **закрыто, при перепроверке не подтвердилось.** `AssessmentQuestionsService.deleteQuestion` (§16) делает soft-delete (`deletedAt`), не физический `DELETE` — ограничение в приложении никогда не срабатывает, тот же паттерн, что уже находили для User/Group (§13).
...
**Edge cases:** проверены (2026-08-08).
- Уникальность `[organizationId, courseId, userId]` теряет историю при пересдаче — подтверждено буквально: `AssessmentAttemptsService.createAttempt` → `certificate.upsert` при повторном `pass` перезаписывает `assessmentAttemptId`/`status`/`revokedAt` существующей записи, `issuedAt` не трогается (остаётся датой первой выдачи) — ссылка на предыдущую попытку теряется. Не баг сейчас: фичи периодической пересдачи (`Due Date Policy/Recurrence`, п. 3.5) не существует вообще, это осознанное упрощение под текущий объём, не забытый недочёт. Действие не требуется, пока recurrence не станет реальной фичей.
- Нет модели `Certificate Template` (п. 4.2 нереализованных) — оформление сертификата зашито вне модели данных. Подтверждено, будущая фича, без изменений.
- `assessmentAttemptId` опционален, нет явного поля «основание выдачи» — подтверждено буквально: `issueCertificate` поддерживает два разных пути (`findPassedAssessmentAttemptId` — по тесту, `isCourseCompleted` — по завершению курса без теста) без различающего поля.
- **Дополнительно проверено (не было в списке):** RBAC «learner не может выдать сертификат сам себе» — подтверждено. `certificatesCreate` в `roles.ts` не включает learner, `RolesGuard` блокирует запрос раньше, чем выполнится код сервиса; внутренняя проверка `targetUserId === currentUserId` в `ensureCertificateAccess` относится к другому сценарию (привилегированный актор выдаёт сертификат сам себе) и не открывает дыру для learner.
...
**RBAC:** нет отдельной строки в публичной RBAC-матрице — служебная модель, заполняется системой при удалении файла материала. Подтверждено (2026-08-08): read-эндпоинта для этого лога нет вообще — единственное использование модели во всём коде — `create` внутри `clearUploadedFile` (`course-materials.service.ts`). Лог чисто write-only.

**Edge cases:** проверены (2026-08-08).
- **Известный, зафиксированный как открытый в `docs/CONCERNS.md` пробел** (запись от 2026-08-06): это единственный существующий аудит-лог в системе — покрывает только удаление файлов материалов, но не остальные значимые действия (смена ролей, публикация курсов, попытки тестов, выдача сертификатов), которые требует `docs/MVP_SCOPE_LOCK.md` как критерий готовности MVP. Подтверждено, `grep` по коду на `notification`/`audit.log` — ноль совпадений. Та запись в `CONCERNS.md` explicitly требует решения владельца продукта (реализовать общий audit log или официально снять из критериев MVP) — не решается правкой документации, §21 дублирует уже отслеживаемый пункт, действие здесь не требуется.
- ~~Нет `actorId` внешнего ключа с `onDelete`-политикой на `User`~~ — **закрыто, при перепроверке не подтвердилось как проблема.** Связи действительно нет (`actorId String` без `@relation`) — но это стандартный, правильный паттерн для audit-лога: намеренно без `Restrict` (не блокировать удаление пользователя из-за истории аудита) и без `Cascade` (не терять запись аудита вместе с актором). К тому же физического удаления `User` в приложении нет вообще (см. §13) — риск чисто теоретический.

---

## Сквозные наблюдения по уже реализованной части схемы

1. **Единый паттерн soft-delete:** почти все 21 модель используют `deletedAt: DateTime?` вместо физического удаления — **уточнено (2026-08-08): исключений четыре, не три** — `Session` (только `revokedAt`), `MaterialFileDeletionAudit`/`Membership` (append-only, без `deletedAt` вовсе) и **пропущенный ранее `MultipartUpload`** (свой `status` pending/completed/aborted вместо `deletedAt`, тоже append-only по духу). Это стоит держать в уме как эталон для 99 нереализованных сущностей — там, где нужна история (Completion Record, Consent Record и т.д.), паттерн append-only без `deletedAt` ближе к правильному, чем soft-delete.
...
3. **`CourseAccessGuard` — единственный объектный guard** в системе, покрывающий 8 контроллеров вокруг курса (подтверждено, число не изменилось: `assessment-attempts`, `assessment-questions`, `assessments`, `assignments`, `certificates`, `course-materials`, `lessons`, `progress`). **Обновлено (2026-08-08):** `CourseResource` union расширен до 9 типов ресурсов — добавлен `'option'` (`AssessmentAnswerOption`, join в 3 уровня: question → assessment → course) при реализации PATCH/DELETE вариантов ответа. Любая новая сущность, вложенная в курс (Module, Question Bank и т.п. из списка 99), потребует либо расширения этого guard, либо явного повторения его логики — рассинхронизация здесь уже один раз создавала open concern (двойной запрос в `assertResourceAccess`, закрыт).
...
5. ~~Единственный enum-статус попытки теста (`completed`) — сигнал, что модель прохождения тестов спроектирована как «одномоментная сдача», без черновиков/пауз~~ — **закрыто (2026-08-08).** Больше не так: добавлен статус `in_progress` при реализации `timeLimitMinutes` (§18) — `POST /assessments/:id/attempts/start` создаёт `in_progress`-попытку с серверным `startedAt`, отправка переводит её в `completed`. Используется только для таймированных тестов; untimed (дефолт, подавляющее большинство) по-прежнему одномоментны. Остаточное ограничение: брошенная `in_progress`-попытка не авто-финализируется (см. §18) — не то же самое, что «сохранить и продолжить позже» в общем виде, но принципиальный барьер (единственный статус) снят.

---

*Документ основан на реальном состоянии `apps/api/prisma/schema.prisma` и `docs/API_RBAC_MATRIX.md` на 2026-08-08 — все 21 секция (§1–§21) и сквозные наблюдения перепроверены построчно против кода в этот день. При любых последующих миграциях схемы или изменениях `roles.ts` документ может устареть — сверяться с кодом, не с этим файлом, как того требует `CLAUDE.md`.*
