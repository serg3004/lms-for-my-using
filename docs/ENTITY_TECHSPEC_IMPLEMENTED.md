...
- Удаление последней `admin`-роли в организации — схема не запрещает организацию остаться совсем без администратора; нужна проверка на уровне бизнес-логики, а не только БД.
...
**Edge cases:**
- И `userId`, и `groupId` опциональны одновременно — схема физически допускает `Assignment` без цели (оба `null`); валидация «ровно одно из двух должно быть заполнено» должна быть на уровне приложения, БД её не гарантирует.
