...
#### PR 137 — Stabilize the expired-cookie refresh e2e test
**Проблема:** `apps/e2e/tests/login-role-redirect.spec.ts` → `an expired access cookie is refreshed before the protected route renders` падал в CI трижды (два раза при таймауте `expect.poll` 5с, один раз уже после увеличения до 10с) — каждый раз с разным симптомом (`refreshSucceeded: false` / `expiredAccessRejected: false`), то есть разный шаг цепочки `401 → POST /auth/refresh → повтор запроса` не укладывался в таймаут в разные разы. Простое увеличение таймаута (5с → 10с, уже сделано) снизило частоту, но не убрало флак полностью.  
**Что делать:** Переписать тест на последовательные `page.waitForResponse()` под каждый сетевой раунд-трип вместо одного `expect.poll`, опрашивающего накопленный массив статусов — это даст точную диагностику, какой конкретно запрос не пришёл, вместо общего таймаута на всю цепочку. Также стоит проверить, не вызвана ли часть флака контентией ресурсов CI-раннера при параллельном запуске (`playwright test` использует 2 воркера, другие тесты в это же время создают/удаляют курсы через ту же тестовую БД).  
**Риск без него:** Продолжит периодически ронять CI на PR, не связанных с auth-кодом, вынуждая делать ре-раны вручную.
