...
export type Assessment = { id: string; title: string; slug: string; description: string | null; passingScore: number; maxAttempts: number | null; availableAfterCourseCompletion: boolean; randomizeOrder: boolean; status: string };
...
export type AssessmentForm = { title: string; description: string; passingScore: string; maxAttempts: string; availableAfterCourseCompletion: boolean; randomizeOrder: boolean; status: AssessmentStatus };

export const ASSESSMENT_STATUSES: AssessmentStatus[] = ['draft', 'published', 'archived'];
export const emptyAssessmentForm = (): AssessmentForm => ({ title: '', description: '', passingScore: '70', maxAttempts: '', availableAfterCourseCompletion: true, randomizeOrder: false, status: 'draft' });
...
export type ValidAssessmentForm = { title: string; slug: string; description: string | null; passingScore: number; maxAttempts: number | null; availableAfterCourseCompletion: boolean; randomizeOrder: boolean; status: AssessmentStatus };
export function mapAssessmentForm(form: AssessmentForm): ValidAssessmentForm | null {
  ...
  return { title, slug, description: form.description.trim() || null, passingScore, maxAttempts, availableAfterCourseCompletion: form.availableAfterCourseCompletion, randomizeOrder: form.randomizeOrder, status: form.status };
}
