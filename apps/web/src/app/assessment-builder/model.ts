...
export type Assessment = { id: string; title: string; slug: string; description: string | null; passingScore: number; maxAttempts: number | null; availableAfterCourseCompletion: boolean; passMessage: string | null; failMessage: string | null; status: string };
...
export type AssessmentForm = { title: string; description: string; passingScore: string; maxAttempts: string; availableAfterCourseCompletion: boolean; passMessage: string; failMessage: string; status: AssessmentStatus };

export const ASSESSMENT_STATUSES: AssessmentStatus[] = ['draft', 'published', 'archived'];
export const emptyAssessmentForm = (): AssessmentForm => ({ title: '', description: '', passingScore: '70', maxAttempts: '', availableAfterCourseCompletion: true, passMessage: '', failMessage: '', status: 'draft' });
export const assessmentToForm = (assessment: Assessment): AssessmentForm => ({ title: assessment.title, description: assessment.description ?? '', passingScore: String(assessment.passingScore), maxAttempts: assessment.maxAttempts === null ? '' : String(assessment.maxAttempts), availableAfterCourseCompletion: assessment.availableAfterCourseCompletion, passMessage: assessment.passMessage ?? '', failMessage: assessment.failMessage ?? '', status: assessment.status as AssessmentStatus });
...
export type ValidAssessmentForm = { title: string; slug: string; description: string | null; passingScore: number; maxAttempts: number | null; availableAfterCourseCompletion: boolean; passMessage: string | null; failMessage: string | null; status: AssessmentStatus };
export function mapAssessmentForm(form: AssessmentForm): ValidAssessmentForm | null {
  ...
  return { title, slug, description: form.description.trim() || null, passingScore, maxAttempts, availableAfterCourseCompletion: form.availableAfterCourseCompletion, passMessage: form.passMessage.trim() || null, failMessage: form.failMessage.trim() || null, status: form.status };
}
