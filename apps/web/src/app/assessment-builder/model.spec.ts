...
describe('assessment builder model', () => {
  it('maps and normalizes a valid form', () => {
    expect(mapAssessmentForm({ ...emptyAssessmentForm(), title: '  Safety Basics  ', description: '  Intro ', maxAttempts: '3' })).toEqual({ title: 'Safety Basics', slug: 'safety-basics', description: 'Intro', passingScore: 70, maxAttempts: 3, availableAfterCourseCompletion: true, passMessage: null, failMessage: null, randomizeOrder: false, status: 'draft' });
  });
  ...
  it('initializes edit values and resets reducer state', () => {
    const form = assessmentToForm({ id: 'a', slug: 'a', title: 'A', description: null, passingScore: 80, maxAttempts: null, availableAfterCourseCompletion: false, passMessage: 'Nice work', failMessage: null, randomizeOrder: true, status: 'published' });
    expect(form).toMatchObject({ title: 'A', description: '', passingScore: '80', maxAttempts: '', passMessage: 'Nice work', failMessage: '', randomizeOrder: true, status: 'published' });
