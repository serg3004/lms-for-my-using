...
export function AdminOrgStructureToolsPage() {
  ...
  const { state, reload } = useAsyncData(() => listOrgStructureHistory(historyPage), [t, historyPage], { unauthenticated: 'Your session expired.', error: 'Unable to load organization history.' });
  ...
  return <AdminPageLayout brandLabel="LearnSpace" sidebarLabel="Admin navigation" navItems={nav} currentUser={currentUser ?? undefined}>
    ...
    <section className="admin-card"><h2>Change history</h2>{state.status === 'loading' ? <PageState variant="loading" message="Loading history…" /> : state.status !== 'loaded' ? <PageState variant="error" message={state.message} /> : <>
      ...
      <p role="navigation" aria-label="History pages">
        <Button type="button" onClick={() => setHistoryPage((p) => p - 1)} disabled={historyPage <= 1}>Previous</Button>{' '}
        <span>Page {state.data.page} of {Math.max(1, Math.ceil(state.data.total / ORG_STRUCTURE_HISTORY_PAGE_SIZE))}</span>{' '}
        <Button type="button" onClick={() => setHistoryPage((p) => p + 1)} disabled={historyPage * ORG_STRUCTURE_HISTORY_PAGE_SIZE >= state.data.total}>Next</Button>
      </p>
    </>}</section>
  </AdminPageLayout>;
}
